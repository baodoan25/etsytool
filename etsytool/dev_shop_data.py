...
def _anh_dai_dien (ten_shop ,mau ):
    chu_cai_dau ="".join (part [:1 ]for part in ten_shop .replace ("-"," ").split ()[:2 ]).upper ()or ten_shop [:2 ].upper ()
    svg =(
    f'<svg xmlns="http://www.w3.org/2000/svg" width="160" height="160" viewBox="0 0 160 160">'
    f'<rect width="160" height="160" rx="34" fill="{mau }"/>'
    f'<circle cx="118" cy="38" r="36" fill="rgba(255,255,255,0.22)"/>'
    f'<text x="50%" y="54%" dominant-baseline="middle" text-anchor="middle" '
    f'font-family="Arial, sans-serif" font-size="42" font-weight="700" fill="white">{chu_cai_dau }</text>'
    f'</svg>'
    )
    return f"data:image/svg+xml;charset=UTF-8,{quote (svg )}"


def _tao_slug_tieu_de (tieu_de ):
    return "".join (character .lower ()if character .isalnum ()else "-"for character in tieu_de ).strip ("-").replace ("--","-")


def _anh_san_pham (tieu_de ,mau ):
    tieu_de_an_toan =tieu_de [:34 ]
    svg =(
    ...
    f'<rect x="34" y="34" width="352" height="252" rx="24" fill="{mau }"/>'
    ...
    f'<text x="70" y="138" font-family="Arial, sans-serif" font-size="26" font-weight="700" fill="white">{tieu_de_an_toan }</text>'
    f'</svg>'
    )
    return f"data:image/svg+xml;charset=UTF-8,{quote (svg )}"


def _san_pham_ban_chay (slug_shop ,ma_listing_goc ,cac_tieu_de ,gia_goc ,doanh_so_bat_dau ,mau ="#f58220",url_dau_tien =None ):
    cac_san_pham =[]

    for chi_muc ,tieu_de in enumerate (cac_tieu_de [:4 ]):
        ma_listing =str (ma_listing_goc +chi_muc )
        url =url_dau_tien if chi_muc ==0 and url_dau_tien else (
        f"https://www.etsy.com/shop/{slug_shop }"
        f"?search_query={quote (tieu_de )}"
        )
        cac_san_pham .append (
        {
        ...
        "title":tieu_de ,
        "listingUrl":url ,
        "listingUrlVerified":bool (chi_muc ==0 and url_dau_tien ),
        "shopSearchUrlVerified":not bool (chi_muc ==0 and url_dau_tien ),
        "imageUrl":_anh_san_pham (tieu_de ,mau ),
        "price":round (gia_goc *(1 +chi_muc *0.16 ),2 ),
        "sales":max (1 ,int (doanh_so_bat_dau /(chi_muc +1 ))),
        "favorites":max (1 ,int (doanh_so_bat_dau *1.8 /(chi_muc +2 ))),
        "shopSlug":slug_shop ,
        }
        )
...
"shopAvatarUrl":_anh_dai_dien ("Craftcornerclub","#f59e9e"),
"createdAt":"Aug 2024",
"country":"US",
...
"bestSellers":_san_pham_ban_chay (
"Craftcornerclub",
2001101 ,
...
"shopUrlVerified":True ,
"shopAvatarUrl":_anh_dai_dien ("SeedCult","#111827"),
"createdAt":"Oct 2018",
"country":"US",
...
9800 ,
"#111827",
url_dau_tien ="https://www.etsy.com/listing/1078899099/sacred-lotus-bonsai-seeds-pink-nelumbo?ref=shop_home_feat_2&pro=1&sts=1&logging_key=87475a4d1f551b490021bc139fa74231cce4d6ac%3A1078899099",
),
},
{
...
"shopUrlVerified":True ,
"shopAvatarUrl":_anh_dai_dien ("CaitlynMinimalist","#b08968"),
"createdAt":"Nov 2014",
"country":"US",
"category":"jewelry",
"listingCount":2700 ,
...
"shopUrlVerified":True ,
"shopAvatarUrl":_anh_dai_dien ("SilverRainSilver","#9ca3af"),
"createdAt":"Apr 2015",
"country":"GB",
"category":"jewelry",
"listingCount":6600 ,
...
"shopUrlVerified":True ,
"shopAvatarUrl":_anh_dai_dien ("PricklyPatchTx","#111827"),
"createdAt":"May 2024",
"country":"US",
"category":"craft-supplies-and-tools",
"listingCount":514 ,
...
"shopUrlVerified":True ,
"shopAvatarUrl":_anh_dai_dien ("OuferJewelry","#e5e7eb"),
"createdAt":"Nov 2019",
"country":"US",
"category":"jewelry",
"listingCount":2100 ,
...
"shopUrlVerified":True ,
"shopAvatarUrl":_anh_dai_dien ("SeedTherapy","#0f172a"),
"createdAt":"Sep 2019",
"country":"US",
...
"shopUrlVerified":True ,
"shopAvatarUrl":_anh_dai_dien ("CuteLittleFabricShop","#14b8a6"),
"createdAt":"Aug 2015",
"country":"US",
"category":"craft-supplies-and-tools",
"listingCount":16300 ,
